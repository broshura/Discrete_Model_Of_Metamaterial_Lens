...
from Parameters import a, a1, Radius, Rings

cnt_sign = 0        # Counter of rings with sign errors
cnt_abs = 0         # Counter of rings with large absolute error

# Matrix produced from verified code
# To compare is necessary to multiply at normalized length
...
with open("DATA/Data.txt", "r") as res:
    RES = res.read()
    Strings = RES.split("\n")[:len(RES.split("\n")) - 1]
    M2 = [[float(k)*(a1/a) ** 1 for k in x.split(" ")] for x in Strings]
